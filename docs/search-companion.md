...
- **Schema version 7** is required (`SCHEMA_VERSION` in this script, `SEARCH_REQUIRED_SCHEMA_VERSION` in `src/search/types.ts`, bumped together). A companion reporting a lower `schemaVersion` is flagged `rebuildRequired` by the client. Every migration runs automatically on companion startup and none of them re-reads a note or recomputes a vector:
  ...
  - **5→6** — pins `chunks_fts.rowid` to `chunks.rowid` (an FTS rebuild; `user_version`-cookied because rowid pinning leaves no structural trace to detect).
  - **6→7** — the entity facet: one additive `ALTER TABLE chunks ADD COLUMN entities TEXT NOT NULL DEFAULT ''` plus a `chunks_fts` drop/refill to add the indexed `entities` column (FTS5 has no `ADD COLUMN`). A note's `author:` frontmatter (string or list) is indexed at a strong bm25 weight, so searching a person's name finds notes they authored even when the name never appears in the body. Verified on the live 53,148-chunk index: every row, embedding and rowid pinning intact, nothing re-embedded by the migration itself.

  **Upgrading across a schema bump means updating the plugin and rebuilding the companion image together.** Between the two, `/health` reports `ok: false` and search is *unavailable*, not degraded — the client refuses an index it cannot rely on rather than serving it silently. Rebuild the container in the same landing as the plugin update. One schema-7 consequence to expect: the client folds the emitted entity text into each note's content hash, so the first indexing sweep after the upgrade re-upserts (and, with semantic indexing on, re-embeds) the whole vault once — that is what populates the facet for existing notes.
- **Ranking modes.** `POST /v1/search` accepts an optional `rankingMode`: `current` (strict chunk-level AND, loose-OR only as a zero-hit rescue), `blend` (always union the loose-OR pool in), `coverage` (a document-level term-coverage retrieval leg fused as a fourth RRF rank), or `blend+coverage`. An unknown value is an HTTP 400, never a silent fallback. **The default is `coverage`**, chosen by a measured four-mode bake-off (eval-harness `measurements/fsq-bakeoff-2026-07-26/`): it was the only mode that improved every quality metric with zero rank-1 regressions, fixing the "all my terms are in that note, why doesn't it match" failure where a query's terms are split across a note's chunks. `blend` measured net-negative as a default and remains per-request only.
