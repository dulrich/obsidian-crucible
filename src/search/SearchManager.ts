...
import {
	buildPreparedSearchFileChunks,
	PreparedSearchFile,
	prepareSearchFile,
	SearchFilePreparationContext,
} from './filePreparation';
...
export class SearchManager {
	...
	/**
	 * The file-preparation subsystem's dependency shape, rebuilt per call rather than cached:
	 * `imageDescriptions` is injected after construction (see `setImageDescriptionStore`), so a
	 * context built once at construction time would permanently see `null`.
	 */
	private preparationContext(): SearchFilePreparationContext {
		return {
			app: this.app,
			settings: this.settings,
			imageDescriptions: this.imageDescriptions,
			isExcluded: path => this.isExcludedFromIndex(path),
		};
	}

	private async prepareFile(file: TFile): Promise<PreparedSearchFile | null> {
		return await prepareSearchFile(this.preparationContext(), file);
	}

	// The chunker call is a pure projection of the prepared file plus the two chunk-size
	// settings, so it lives beside the preparation that produced `prepared` — see
	// `buildPreparedSearchFileChunks`.
	private buildPreparedFileChunks(prepared: PreparedSearchFile): SearchChunk[] {
		return buildPreparedSearchFileChunks(this.preparationContext(), prepared);
	}
